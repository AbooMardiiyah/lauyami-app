...
class AsyncQdrantVectorStore:
    """Manages asynchronous interactions with Qdrant vector store for legal document ingestion.
    ...
    Attributes:
        ...
        splitter (TextSplitter): Utility for splitting document content into chunks.
    ...
    def __init__(self, cache_dir: str | None = None):
        ...
        self.document_batch_size = vector_db.document_batch_size
    ...
    async def _document_batch_generator(
        self, session: Session, from_date: datetime | None = None
    ) -> AsyncGenerator[list[LegalDocument], None]:
        """Yield batches of legal documents from SQL database.

        Args:
            session (Session): SQLAlchemy session for querying documents.
            from_date (datetime, optional): Filter documents from this date.

        Yields:
            list[LegalDocument]: Batch of legal documents.
        ...
        try:
            ...
            while True:
                ...
                if not documents:
                    break
                yield documents
                offset += self.document_batch_size
        except Exception as e:
            self.logger.error(f"Failed to fetch document batch: {e}")
            raise

    async def ingest_from_sql(self, session: Session, from_date: datetime | None = None):
        """Ingest legal documents from SQL database into Qdrant vector store.

        Fetches documents in batches, generates embeddings, and upserts them to Qdrant.
        Skips existing documents and logs throughput.
        ...
        try:
            ...
            async for documents in self._document_batch_generator(session, from_date=from_date):
                ...
                for start in range(0, len(all_chunks), self.upsert_batch_size):
                    ...
                    self.logger.info(
                        ...
                        f"Total documents: {total_documents}, Total chunks: {total_chunks}"
                    )
            ...
            self.logger.info(
                f"Ingestion complete: {total_documents} documents, {total_chunks} chunks, "
                f"final average speed = {final_speed:.2f} chunks/sec"
            )
        except Exception as e:
            self.logger.error(f"Failed to ingest documents to Qdrant: {e}")
