...
def log_batch_status(
    ...
    action: str,
    batch_size: int,
    total_articles: int | None = None,  # Kept for backward compatibility, but use total_documents
    total_chunks: int | None = None,
    context: str = "",
    total_documents: int | None = None,  # New parameter name for legal documents
) -> str:
    ...
    Args:
        ...
        total_articles (int | None): Total articles/documents processed so far (deprecated, use total_documents).
        total_chunks (int | None): Total chunks processed so far.
        context (str, optional): Additional context info.
        total_documents (int | None): Total documents processed so far (preferred for legal documents).
    ...
    # Use total_documents if provided, otherwise fall back to total_articles for backward compatibility
    total_docs = total_documents if total_documents is not None else total_articles

    details = (
        f"{action} | batch_size={batch_size}"
        f"{f', total_documents={total_docs}' if total_docs is not None else ''}"
